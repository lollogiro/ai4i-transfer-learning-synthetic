...
For each of the three initial domain configurations (Component, Load, Seasonal), geometric distances were computed on the anomalies in the 7-D continuous space using a shared, symmetric scaler. Interestingly, this revealed a critical decoupling between distance and penalty: while the Operational shift (Load) produced the highest spatial distance, it yielded near-zero transfer penalty because the uniform baseline shift did not alter the physical anomaly signature. Conversely, the Seasonal shift (Spring $\rightarrow$ Summer) physically rotated the decision boundary, causing catastrophic model failure (54% Penalty).
To isolate this boundary-breaking drift and rigorously quantify the correlation between distance and penalty, we constructed 24 strictly disjoint temporal permutations (1-Month and 1-to-2-Month shifts) from the seasonal data, completely avoiding data leakage. Across this temporal dataset, all three spatial metrics showed a strong, statistically significant positive correlation with the transfer performance drop: **SWD** ($\rho=+0.706$ [+0.408, +0.866]), **FD** ($\rho=+0.645$ [+0.325, +0.846]), and **MMD** ($\rho=+0.625$ [+0.274, +0.818]). Applying symmetric scaling allowed the MMD kernel to accurately capture the distributional shifts, confirming that both mean-shift and covariance-shape distances reliably predict transfer degradation when the anomaly boundary physically drifts.
