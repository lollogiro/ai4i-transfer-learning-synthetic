...
Distances are computed on **source-standardized** features: the per-class scaler is fit on the source side and applied to both domains. The distance is therefore *directional*, measuring how far the target sits from the source's reference frame, mirroring the transfer itself, since a model trained on the source never uses target statistics when fitting. Under this normalization, across the previously defined 20 source-target pairs we observed a strong correlation between the geometric distance and the transfer penalty. The Spearman coefficients were **SWD = +0.46** [95% CI 0.09, 0.76], **FD = +0.42** [0.01, 0.74], and **MMD = -0.31** [-0.70, 0.11]. The transfer penalties ranged from 0.06 to 0.74, and pairing these results with corresponding domain distances, we found that the performance loss when transferring models, in most cases, also increases with the distance between source and target domains. The correlation was statistically significant for SWD and FD, while MMD failed to show a significant relationship, confirming that not all distance metrics are equally effective in predicting transferability.

Because a source-only fit makes the distance asymmetric ($\mathrm{dist}(A \to B) \neq \mathrm{dist}(B \to A)$), we repeated the same correlation under a **symmetric normalization**, where the per-class scaler is fit on the union of the two domains, making the distance order-independent. Under this variant the predictive power essentially disappears: SWD $\rho = -0.03$, FD $\rho = -0.04$, MMD $\rho = +0.37$, all with confidence intervals crossing zero. The two normalizations answer different questions. The symmetric distance measures an *intrinsic* dissimilarity between the two domain distributions. The source-only distance instead measures how the target is displaced *with respect to the source*, the quantity that decides whether a classifier trained on the source will generalize to the target. Since transfer learning is a directed operation (a model is trained on the source and applied to the target), the source-anchored, directional distance can be considered an appropriate predictor, and the intrinsic distance is not predictive on its own. In line with this, SWD and FD predict the transfer penalty only under the directional normalization, while MMD is not a reliable predictor under either.
...
- **Penalty tracks the distance**: as N grows, the conditional SWD/FD to the target test decreases and the penalty partially follows this trend.
